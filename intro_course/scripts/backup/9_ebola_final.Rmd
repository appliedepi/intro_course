...
# surveillance
surv_raw <- import(here("surveillance_linelist_20141201.csv"))
...
```{r}
# Filter date_report before the publish_date parameter (defined in YAML heading), and KEEPS missing date_report
combined <- combined %>% 
  filter(date_report <= ymd(params$publish_date) | is.na(date_report))
```
