...
ggplot(linelist, mapping = aes(x = epiweek, fill = fct_lump_n(city, 5))) +
  geom_histogram() +
  scale_y_continuous(
  ...
  labs(title = "COVID-19 epidemic, Fulton County, GA",
       ...
       y = "Number of cases",
       fill = "City",
       caption = str_glue("Fake data for course exercise. {nrow(linelist)} cases shown by city of residence\nCases reported from {min(linelist$date_report)} to {max(linelist$date_report)}")) + 
