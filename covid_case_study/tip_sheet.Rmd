...
- Remember to convert your date columns to class date using one of the {lubridate} functions, `dmy()`, `mdy()`, or `ymd()` based on the format of the dates *prior* to applying the function.  

- Consider creating a `data_date` or `report_date` object that you use as a cutoff (or with a 7 day delay). You could define this in the YAML
...
```{r, echo=T, eval=F}
ggplot(data = combined, mapping = aes(x = date_report, fill = fct_lump_n(hospital, 5)))+
     geom....
```
