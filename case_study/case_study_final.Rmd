...
Here we are going to define the report parameters, such as the epiweek to run the report for, and the districts to run the report for.

data-date is `r data_date`.
...
monday_floor <- floor_date(min(combined$date_onset, na.rm=T),   "week", week_start = 1)

# Monday after last case
monday_ceiling <- ceiling_date(max(combined$date_onset, na.rm=T), "week", week_start = 1)
...
ggplot(data = combined)+
       ...
       labs(
         ...
         caption  = stringr::str_glue("n = {nrow(combined)} combined; Hospitalisations range from {format(min(combined$date_onset, na.rm=T), format = '%a %d %b %Y')} to {format(max(combined$date_onset, na.rm=T), format = '%a %d %b %Y')}\n{nrow(combined %>% filter(is.na(date_onset)))} combined missing date of onset and not shown"))
```
