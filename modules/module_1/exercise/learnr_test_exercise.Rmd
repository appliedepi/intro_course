...
This exercise will guide you through a set of tasks. *You should perform these tasks in RStudio, on your local computer*.  
...
Please complete the quiz questions that you encounter throughout the tutorial.  

* **Answering will help you** to comprehend the material, and will also help us to improve the exercises for future students.  
...
question_numeric(
 "How anxious are you to begin this tutorial, on a scale from 1 (least anxious) to 10 (most anxious)?",
...
* Practice opening a new R script and running code "commands"
* Learn basic syntax and operators in R  
* Install and load R packages  
* Import an Excel dataset into R  
* Review a dataset in R  
...
* It is open-source (capabilities are created by the user community)  
* Its large and vibrant user community.  
...
When you install R to your computer, you can open it and type commands into the R Console window that appears. Using R this way will run all the computations and create all the plots you desire, but it will be difficult to stay organized. Now, people use Integrated Development Environments (IDEs) such as RStudio to wrap-around their R software, allowing a more friendly user interface and file organization.  


### RStudio is the vehicle  

"RStudio" is an interface (IDE) for easier use of R. It is offered for free by the RStudio corporation. You can think of R as being the engine of a vehicle, doing the crucial work, and RStudio as the body of the vehicle (with seats, accessories, etc.) that helps you actually use the engine to move forward! Therefore, you should install R *and* RStudio to proceed.  
...
knitr::include_graphics(here::here("images", "functions_packages", "RStudio-overview.PNG"))
```
...
If you do not work in an R project, it will be harder to organize files, to share your work with others, and you may encounter hours of frustration struggling to import files with something called the "working directory". When you use an R project, your scripts become much more easy to share and less likely to "break" when someone else uses them.  
...
knitr::include_graphics(here::here("images", "functions_packages", "Rproject_dropdown.png"))
```
...
knitr::include_graphics(here::here("images", "functions_packages", "create_project.png"))
```
...
**Open a new R script** by clicking *File -> New file -> R script*. You should see a new empty tab appear in the upper-left pane of RStudio.  

```{r script-new, fig.margin = TRUE, echo = FALSE, fig.width = 3, out.width = "50%", fig.cap = ""}
knitr::include_graphics(here::here("images", "functions_packages", "new_script.jpg"))
```
...
A typical script for public health analysis/reporting might include topics like:  
...
knitr::include_graphics(here::here("images", "functions_packages", "example_script.png"))
```


### Open a new R Script  

Once the file has been opened, click the "Save" button (or *File -> Save*), name the file "demo.R", and save it into the RStudio project folder.
...
Below are common mathematical operators in R. These are often used to perform addition, division, to create new columns in datasets, etc. Whether you put spaces around the operators is not important.
...
**Type simple mathematical commands into the R script and try all of the above ways to run a command. Which is your preferred method?**  
...
**Run commands in your R script to find the solutions to these math problems:**  
...
You are managing a COVID testing campaign across 3 sites, and you need to place a supply order for rapid tests for next month. Based on the following information, how many tests do you need to order?  
...
In a given work session you may create ("define") and re-define hundreds of objects. Some examples of objects include:  
     ...
     * A dataset (imported into R from an Excel, CSV, or other file format)
...
We have created the object `confirmed_cases`, and assigned it the value `34`.  

```{r, quiz-assign, echo=F}
quiz(
   question_checkbox("When you run the command above, what happens (select ALL that are correct)?",
    answer("A new stored object appears in the Environment pane, named 'confirmed_cases', with a value of 34",
           correct = TRUE,
           message = "RStudio is an 'Integrated Development Environment (IDE) that makes it easier to write, use, debug, and save R code."),
    answer("The command that was run appears in the Console (lower-left) pane",
           correct = TRUE,
           message = "Note that only the command appears as run, not any calcuated output"),
    answer("34 appears in the Console as calculated output",
...
`r icons::icon_style(icons::fontawesome("terminal", style = "solid"), scale = 2, fill = "black")` 
Back to your R script... you can now return/print the *value* assigned to the name `confirmed_cases`, by simply running the name as a command (look at the R console pane):  
...
What happens if you receive news that there are 10 additional confirmed cases?  If you modify the first command to read `confirmed_cases <- 44`, does the value of `total_cases` in the RStudio environment change to `56`?  

```{r, quiz-case2-math, echo=F}
quiz(
   question_checkbox("Which steps must happen for total_cases to be updated to 56?",
    answer("Run confirmed_cases <- 44",
           message = "Yes, first the confirmed_cases must be re-defined",
    correct = T),
    answer("Run total_cases <- confirmed_cases + suspect_cases",
...
If you change a written value in your script, it does not update the rest of your script, nor does it change any values stored in R!  
You must re-run the commands in order for the changes to be registered. In this case you must run two commands to update the value of `total_cases` (and they must be run in the correct order!)  
...
2) If you want to run ALL the commands in your script, from top-to-bottom, click the "drop-down menu next to "Source" button to the right of "Run".  
...
You wrote this command to reflect the need for tests next month:  
...
Using the `#` symbol, add comments into the script that clearly explain what is occuring. As described in the class demonstration, any text written to the right of the comments is ignored by R as it runs commands. So, you can place a command at the beginning of a line, or you can place the comment after some code (to write a comment on the same line as code).  
...
Time for you to practice in RStudio. In your "demo.R" script, use the function `min()` and `max()` to find the minimum and maximum of the number 3, 55, 9, -4, and 33.  
...
Try this in your R script. What happens when you print `patient_ages`? All the numbers appear.  
...
Now, I'm not sure why we would need to multiply patient ages by 2, but it sure was easy, wasn't it?!
...
What can we do with a character vector? We cannot multiply it by 2... for fun, let us put the vector in another function `toupper()`, which changes all of the characters to upper case:  
...
### Functions with names arguments  
...
knitr::include_graphics(here::here("images", "functions_packages", "arguments-buttons.png"))
```
...
Imagine you have this vector of jurisdictions, but when you run `jurisdictions` in R to print the list of jurisdictions, it looks ugly and is not very readable.  
...
This function has an **argument** that is named `collapse`. Most arguments are written *within the parentheses with a single equals sign*, so we will refer to this argument as `collapse = `. If there are multiple arguments for one function, they are written with commas between them. See the result:  
...
In the example above, we have chosen ", " which is a comma. The quotation marks ensure that R understands this is a character value.  
...
Often, functions have many arguments... but most have default values. For example, functions that make plots may be able to be adjusted in dozens of ways (`title = `, `subtitle = `, `color = `, etc.). But most of these arguments have **default values**, meaning that you do not need to supply a value for the function to run. Instead, you can write the argument and its assigned value *if you wish*.  

#### Getting help  

A function's arguments and any default values can be read in the function's *documentation*. To read the documentation, search the name of the function in the **Help pane** in the lower-right of RStudio. Alternatively, type `?` before the function name in the RStudio **Console** pane (for example, `?paste`). The details will look something like this (we can help you interpret, as they can be difficult to understand at first):  

```{r, fig.margin = TRUE, echo = FALSE, fig.width = 3, out.width = "100%", fig.cap = ""}
knitr::include_graphics(here::here("images", "functions_packages", "r-help.png"))
```
...
The magic of R happens when you **create your own functions**. This is an advanced skill that we do not cover in this course, and one does not typically need to create functions until they are more experienced in R. However, this aspect of R is where its versatility really begins to shine. Imagine if you could convert your entire workflow into one command?
...
Now that the function has been defined we can run the equation, providing various input values, like this:  
...
Until now, you have used R functions that come installed with R. These are called **base R** functions, and they represent a very small portion of all R functions available to you.  
...
**When you install R packages, most of the time you install directly from CRAN**

Many R users create specialized functions, which they share as packages with the R community to be verified. For packages to be widely distributed, they must be shared on the **Comprehensive R Archive Network (CRAN)**, which is R's central software repository - an archive of R packages that have passed basic scrutiny. As of March 2022, there are 19,020 packages available on CRAN. Some of these are immensely popular, with hundreds of thousands of downloads each month. 
...
*Packages* can be installed (downloaded) to your computer from the internet. Once a package is downloaded, it is stored in **your R “library”**. You can then access the functions it contains during your current R session by “loading” the package for use.
...
```{r, fig.margin = TRUE, echo = FALSE, fig.width = 3, out.width = "100%", fig.cap = ""}
knitr::include_graphics(here::here("images", "functions_packages", "bookshelf1.png"))
```
...
The name of the package to install must be provided in the parentheses and in quotes. If you want to install multiple packages in one command, they must be listed within a character vector `c()`.
...
**We had you install the package "pacman" because it contains functions to make it easier to install and load other functions**.  

I's name refers to "**pac**kage **man**ager", not the video game character.  

You can imagine the complications caused when sharing R scripts with someone else? Do they have the same R packages that you have? You don't want them to have to search through the script to find which R packages you used in the script... should you write `install.packages()` at the top of the script, and then all the `library()` commands as well? If you use 15 packages, that is a lot of lines... and potentially a lot of unnecessary installations...

One solution is **pacman**'s function `p_load()`. You can list the names of packages inside, and it will install each package *only if it is not already installed*. Additionally, it will load each package for use in the R session. It is a very efficient option - you can include it at the top of your script. If your colleague receives the script, as long as he has **pacman** the script will only install the packages he does not already have, and will load all of them for use.  
...
This command can also be written vertically. **In any R function, the newlines and indenting will not impact the execution of the code.** But it can dramatically improve readability!  
...
If you forget them, these can always be found in the Epi R Handbook [R Basics page](https://epirhandbook.com/en/r-basics.html#operators).  
...
To begin working with data, you must import it into the R environment as an object. Once imported, the dataset will be saved as a **data frame** object, which consists of columns and rows.  
...
`r icons::icon_style(icons::fontawesome("window-restore", style = "solid"), scale = 2, fill = "darkgrey")`
Open your File Explorer and copy all the course files into your R project. Below is a screenshot of how they should look.  

<!-- TODO screenshot   -->

Do you see the file "*surveillance_linelist.xlsx*" that is saved in the same folder as the "*intro_course.Rproj*" file? This is the **"root" (baseline) folder** of the R project. To access this folder from the R project file you do not need to click into any sub-directories (sub-folders).  



### rio - the easy package for importing data  

There are **base R** functions for importing data, but they can be confusing and difficult to remember - there are separate functions for each type of file (e.g. .xlsx, .csv, .tsv, .txt, etc). Thankfully, there is one function that works for almost all file formats, which is the `import()` function from the package **rio**.  

In it's most simple form, the `import()` function accepts a character value - the file path to the data that you wish to import. Usefully, if the data file is saved in the root folder, you only need to specify the file name and extension, as below:  

```{r, echo=T, eval=F}
import("surveillance_linelist.xlsx")
```


`r icons::icon_style(icons::fontawesome("exclamation", style = "solid"), scale = 2, fill = "red")` 
Did you see this error?
>Error in import("surveillance_linelist.xlsx") :  could not find function "import"
...
**Think: what did your command ask R to do?** It asked R to import the dataset... and the default action is to print/display it. You did not ask R to save the dataset as a named object. How would you do that?  

**In RStudio, try again to import the data and save it as the object `surv_ll_raw` (a short name for "surveillance linelist raw").** 
...
```{r eval = F, echo=T}
surv_ll_raw <- import("surveillance_linelist_12012014.xlsx")
```
...
You should now see the new object `surv_ll_raw` appear in the R **Environment pane**, under the subcategory "Data". It should say "665 obs. of 19 variables". Congratulations, you have now imported a dataset into R!  
...
A typical file path ("absolute" file path, or a "full" file path) looks like this:  

```{r, echo=T, eval=F}
surv_ll_raw <- import("C:/Users/Laura/Documents/intro_course/data/surveillance_linelist_12012014.xlsx")
```

The problem with this is all of the path before "intro_course" ... the C: ... the "Laura" ... etc. None of these may be present in the computer of Laura's colleague, and this command would not run. The file path would be "broken". Thankfully there is a solution:  
...
It has created the full file path behind the RStudio project root folder ("intro_course"). It is easy to add sub-folders to this, like this:  
...
```{r, echo=T, eval=F}
here("data", "raw", "surveillance_linelist_12012014.xlsx")
```

This would produce the file path:  

```{r, echo=T, eval=F}
"C:/Users/Laura/Documents/intro_course/data/raw/surveillance_linelist_12012014.xlsx")
```
...
surv_ll_raw <- import(here("data", "raw", "surveillance_linelist_12012014.xlsx"))
```
...
quiz(caption = "Quiz - importing",
  question("How would you write the import() and here() command for the following filepath:\n'E:/Mario/My Documents/R stuff/malaria/received/wk22/cases.csv', when the R project is located in the folder 'malaria'?",
    allow_retry = T,
...
For even more detail, collapse this overview and **click on the name of the data frame itself (`surv_ll_raw`).**  
