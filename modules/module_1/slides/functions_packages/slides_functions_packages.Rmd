...
You can reference other values within the equation:  
* Excel function: `sumif(B2, "")`
...
Once again, Excel is not so different:  
  * Arguments have names  
  * Arguments values are separated by commas  
  * Some are required, others are optional  

--

.footnote[What is returned by this Excel equation?]
...
Let's say we have this dataset below. We import it into R, and save with the name `surv`.  

We want to cross-tabulate the columns `case_def` and `gender`.  
...
Most functions you will use must be downloaded in an R **package**.  
...
**Step 2 - Load:** Begin every R session by "loading" the packages you want to use  
  * This is like borrowing that book from your library  
...
This package help you manage other packages.  
...
.pull-left[
"rio" is shorthand for "R inputs/outputs"  
]
...
The `{here}` package to find files
