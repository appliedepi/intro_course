...
```{r eval=FALSE, echo=TRUE}
filter(demo, age < 5 & (gender == "m" | case_def == "Suspect"))
```
...
Before, the 1st argument in `filter()` and in `select()` was the data frame
...
Piping the data through *multiple* functions  

```{r, eval=F, echo=T}
demo %>% filter(age < 5) %>% select(case_id, age, gender)
```

```{r, eval=T, echo=F}
demo %>% filter(age < 5) %>% select(case_id, age, gender) %>% knitr::kable()
```
...
demo %>% 
  select(case_id, age, gender, case_def) %>% 
  distinct() %>% 
  filter(age < 5, case_def == "Confirmed")
```
...
demo %>% 
  ...
  filter(age < 5, case_def == "Confirmed") %>%       # only young children confirmed cases
...
Instead: add the assignment operator **`<-`** to define a new data frame.  
...
* `mutate()` is used to create, or add new columns  
