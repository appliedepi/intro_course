...
```{r, include=FALSE}
# data prep --------------------------------------------------------------------
surv_raw <- rio::import(here::here("data", "surveillance_linelist_12012014.csv"))
```
...
* Change `onset_date` to `date_onset` (it is more standardized to put "date" at the front, especially when there are more date columns)  
...
```{r, echo=T, eval=F}
surv %>% 
  select(case_def, age, gender, case_def)
```
...
**Please note the below commands related to missingness in R,** as they will come in useful later.  

* `is.na()` is used to assess whether a value is `NA`  
* `!is.na()` is used to assess whether a value is NOT `NA`  

These will come in useful when you need to count the number of missing values.  
...
```{r, echo=T, eval=F}
# Export cleaned file  
export(surv, here("data", "clean", "surveillance_linelist_clean.rds"))
```
